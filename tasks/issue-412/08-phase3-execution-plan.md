...
- broker 호출 중 예외가 나면 접수 여부를 단정하지 않고 `UNKNOWN` 저장 후 원래 예외 재전파.
- 원장에는 API key/token 등 요청 secret을 저장하지 않는다. 정규화된 주문 메타데이터와 broker 결과만 저장한다.
...
- 실제 주문 없이 fake broker selftest로 상태 3종(SUBMITTED/FAILED/UNKNOWN) 확인
